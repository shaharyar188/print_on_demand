...
$(document).ready(function () {
    $(document).on("change", "#switch_toogle", function (stop) {
        stop.preventDefault();
        if ($(this).is(":checked")) {
            Swal.fire({
                toast: true,
                icon: "success",
                title: "You Turned ON the OTP Verification..You Verified By the OTP Number for Every Times of Login..!!",
                animation: false,
                position: "top-right",
            ...
            Swal.fire({
                toast: true,
                icon: "success",
                title: "You Turned OFF the OTP Verification..!!",
                animation: false,
                position: "top-right",
                showConfirmButton: false,
                timer: 3000,
...
$(document).ready(function () {
    ...
    $("#campaign_type").change(function () {
        ...
        if (campaign_type === "Schedule") {
            ...
            $(".campaign_date").prop("required", false);
            $("#campaign_time")
                .css({
                    opacity: 0,
                })
            ...
            $(".campaign_time").prop("required", false);
            $("#time_zone")
                .css({
                    opacity: 0,
                })
...
//////////////////////////
const mmstextAreaElement = document.querySelector("#mms_message_details");
const mmscharacterCounterElement = document.querySelector(
    "#mms-character-counter"
);
const mmstypedCharactersElement = document.querySelector(
    "#mms-typed-characters"
);
const mmsmaximumCharacters = 153;
mmstextAreaElement.addEventListener("input", (event) => {
    const mmstypedCharacters = mmstextAreaElement.value.length;
    if (mmstypedCharacters > mmsmaximumCharacters) {
        return false;
    }
    mmstypedCharactersElement.textContent = "Word Count: " + mmstypedCharacters;
});
//////////////////////////
const watsapptextAreaElement = document.querySelector(
    "#watsapp_message_details"
);
const watsappcharacterCounterElement = document.querySelector(
    "#watsapp-character-counter"
);
const watsapptypedCharactersElement = document.querySelector(
    "#watsapp-typed-characters"
);
const watsappmaximumCharacters = 153;
watsapptextAreaElement.addEventListener("input", (event) => {
    const watsapptypedCharacters = watsapptextAreaElement.value.length;
    if (watsapptypedCharacters > watsappmaximumCharacters) {
        return false;
    }
    watsapptypedCharactersElement.textContent =
        "Word Count: " + watsapptypedCharacters;
});
//=====================text-area counting here================================
...
// ===================keywords coading for the campaign =============================
// ===================getting data for the button disabled for the add==========
$(document).ready(function () {
    $(document).on("change", "#group_name", function (stop) {
        stop.preventDefault();
        var value = $(this).val();
        $.ajax({
            url: "/get-data/" + value,
            ...
            success: function (response) {
                if (response.message == 200) {
                    $("#message_details").val("");
                    $("#mms_message_details").val("");
                    $("#watsapp_message_details").val("");
                    $("#insert").empty();
                    $("#insert").attr("disabled", false);
                    $("#insert").append("Add Campaign >");
                    $("#display").text("");
                } else {
                    $("#message_details").val("");
                    $("#mms_message_details").val("");
                    ...
                    $("#display").empty();
                    $("#display").append(
                        "Sorry..! No Contact is assigned to this Group..!!"
                    );
                    $("#insert").attr("disabled", true);
                    $("#insert").text("");
                    $("#insert").append(
                        "<strong class='spinner-border spinner-border-sm' role='status' aria-hidden='true'></strong> Processing..."
                    );
                }
            },
        });
...
// ===================getting data for the button disabled for the email campaign=============
// ===================getting data for the button disabled for the email campaign=============
//////////////////////////////////////
// ===================getting data for the button disabled for the email campaign=============
// ===================getting data for the button disabled for the update==========
$(document).ready(function () {
    $(document).on("change", "#group_name", function (stop) {
        stop.preventDefault();
        var value = $(this).val();
    ...
    $(document).on("change", "#message_type", function (e) {
        ...
        if (message_type == "sms") {
            ...
            $("#mms_message")
                ...
                .slideUp("slow");
            $("#mms_message").prop("required", false);
            $("#mms_message").val("");
            $("#watsapp_message")
                .animate({
                    opacity: 0,
                ...
                .slideUp("slow");
            $("#watsapp_message").prop("required", false);
            $("#watsapp_message").val("");
        } else if (message_type == "mms") {
            $("#value_getting").val("sent_as_mms");
            $("#mms_message")
                .css({
                    opacity: 0,
                })
            ...
            $("#mms_message").prop("required", true);
            $("#sms_message")
                .animate({
                    opacity: 0,
                })
            ...
            $("#value_getting").val("sent_as_watsapp");
            $("#watsapp_message")
                .css({
                    opacity: 0,
                })
            ...
            $("#watsapp_message").prop("required", true);
            $("#sms_message")
                .animate({
                    opacity: 0,
                })
